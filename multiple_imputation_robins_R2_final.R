...
library(dagitty)
library(ggdag)
# install.packages("DescTools")  # if you need it later

# ---- DAG: declare causal structure & fix minimal adjustment set (MSAS) ----
# ---- Global toggles ----
#include_pm25_in_primary  <- FALSE   # Main models EXCLUDE PM2.5
#include_pm25_in_subgroup <- FALSE   # Subgroup/interaction models EXCLUDE PM2.5
...
# Coordinates (same layout each time)
...
setwd("/Users/fati0049/Desktop/My_data_2024/Research/Zohra1/Rcode/analysis_paper2_new/Rcode")

# Load data
LBW <- fread("2025_LBW_data_prop.csv", header = TRUE) %>%
...
population <- fread("/Users/fati0049/Desktop/My_data_2024/Research/Zohra1/Rcode/analysis_paper2/Rcode/population_data_prov.csv", header = TRUE) %>%
...
#####################################
# Multiple imputation (passive Rural)
#####################################
...
#############
# Merge precipitation and scale variables (single scaling block)
############
...
###############################################
# step 2: models with various parameters (Rubin-pooled)
###############################################
...
###############################################
# Province-level RR curves (original °C output)
###############################################
...
#write.csv(prov_medians_df, "Province_Median_Temperature_Reference.csv", row.names = FALSE)
...
#####################
# Province-level RR figure (curve + histogram)
#####################
...
## ===============================
## Heat Vulnerability Index (HVI) – corrected
## ===============================

############################################################
## Heat Vulnerability Index (HVI) – end-to-end script
## - Uses province-level RR at 99th percentile as susceptibility
## - Avoids double counting (HI vs. T/RH/precip)
## - Propagates RR uncertainty on the log scale
############################################################
...
adm2_path <- "/Users/fati0049/Desktop/My_data_2024/Research/Zohra1/Rcode/analysis/pak_adm_wfp_20220909_shp/pak_admbnda_adm2_wfp_20220909.shp"
...
adm1_path <- "/Users/fati0049/Desktop/My_data_2024/Research/Zohra1/Rcode/analysis/pak_adm_wfp_20220909_shp/pak_admbnda_adm1_wfp_20220909.shp"
...
#############
# Projected HVI (drop-in replacement)
#############
...
#############################
# New Subgroup analysis via single interaction model + linear combos
#############################
...
# Combine outputs
pred_by_sub <- bind_rows(res_pm25$preds, res_edu$preds, res_wealth$preds, res_area$preds)
het_tbl     <- bind_rows(res_pm25$het,   res_edu$het,   res_wealth$het,   res_area$het)
